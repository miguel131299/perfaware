...
//-----------------------------------------------------------------------------
// Handler for ADD immediate to accumulator instruction (opcode pattern 0b00000100)
// Supports: immediate-to-AL/AX addressing mode
class ADDImmediateAccumulatorHandler : public InstructionHandler {
public:
  uint32_t decode(std::stringstream &ss, const std::vector<char> &bytestream,
                  uint32_t baseOffset) override;

  [[nodiscard]] std::string_view getName() const override {
    return "ADD (imm to accumulator)";
  }
};
