...
//-----------------------------------------------------------------------------
// Handler for CMP immediate to register/memory instruction (opcode pattern 0x80-0x83)
// Supports: immediate-to-register and immediate-to-memory addressing modes
// REG field must be 7 for CMP (as part of the 0x80-0x83 group)
class CMPImmediateRegMemHandler : public InstructionHandler {
public:
  uint32_t decode(std::stringstream &ss, const std::vector<char> &bytestream,
                  uint32_t baseOffset) override;

  [[nodiscard]] std::string_view getName() const override {
    return "CMP (imm to reg/mem)";
  }
