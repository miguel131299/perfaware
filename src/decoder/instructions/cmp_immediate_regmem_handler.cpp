...
uint32_t CMPImmediateRegMemHandler::handleAddressingMode(
  ...
  case MODEncoding::REGISTER_MODE: {
    ...
    outputCMPInstruction(ss, regOperand, immediate.value);
  ...
  case MODEncoding::MEMORY_MODE_NO_DISPLACEMENT: {
    ...
    if (rmBits == 6) {
      ...
      outputCMPInstruction(ss, memOperand.value, immediate.value);
...
//-----------------------------------------------------------------------------
uint32_t CMPImmediateRegMemHandler::decode(std::stringstream &ss,
  ...
  if (regCode != 7) {
    throw std::runtime_error(
        "Invalid CMP immediate to reg/mem: REG field must be 7, got " +
