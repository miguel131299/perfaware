...
struct GlobalProfiler {
  ...
  int addEntry(const char *label) {
    ...
    int parentIdx =
        parentStackDepth > 0 ? parentStack[parentStackDepth - 1] : -1;
    for (size_t i = 0; i < entryCount; ++i) {
      if (std::strcmp(entries[i].label, label) == 0 &&
          entries[i].parentIndex == parentIdx) {
        return i;
      }
    }
    ...
    std::strncpy(entries[newIndex].label, label,
                 sizeof(entries[newIndex].label) - 1);
  ...
  void printEntriesByDepth(int depth, int parentIndex) {
    for (size_t i = 0; i < entryCount; ++i) {
      if (entries[i].parentIndex == parentIndex) {
        u64 exclusiveCycles =
            entries[i].elapsedCycles - entries[i].elapsedChildrenCycles;
        double exclusivePct =
            totalTime > 0 ? (double)exclusiveCycles / (double)totalTime * 100.0
                          : 0.0;
        double inclusivePct = totalTime > 0 ? (double)entries[i].elapsedCycles /
        ...
        if (entries[i].elapsedChildrenCycles > 0) {
          printf("%s: %llu cycles (%.2f%% excl, %.2f%% incl) [%llu hits]",
                 entries[i].label, (unsigned long long)exclusiveCycles,
                 exclusivePct, inclusivePct,
                 (unsigned long long)entries[i].hitCount);
        } else {
          printf("%s: %llu cycles (%.2f%%) [%llu hits]", entries[i].label,
                 (unsigned long long)entries[i].elapsedCycles, inclusivePct,
                 (unsigned long long)entries[i].hitCount);
        }
  ...
  void printAggregatedResults() {
    ...
    for (size_t i = 0; i < entryCount; ++i) {
      ...
      if (!found && aggregatedCount < MAX_ENTRIES) {
        std::strncpy(aggregated[aggregatedCount].label, entries[i].label,
                     sizeof(aggregated[aggregatedCount].label) - 1);
        aggregated[aggregatedCount]
            .label[sizeof(aggregated[aggregatedCount].label) - 1] = '\0';
        aggregated[aggregatedCount].totalExclusiveCycles = exclusiveCycles;
        aggregated[aggregatedCount].totalHits = entries[i].hitCount;
        aggregated[aggregatedCount].totalProcessedBytes =
            entries[i].processedByteCount;
        aggregatedCount++;
      }
    }
    ...
    for (size_t i = 0; i < aggregatedCount; ++i) {
      for (size_t j = i + 1; j < aggregatedCount; ++j) {
        if (aggregated[j].totalExclusiveCycles >
            aggregated[i].totalExclusiveCycles) {
    ...
    for (size_t i = 0; i < aggregatedCount; ++i) {
      double pct = totalTime > 0 ? (double)aggregated[i].totalExclusiveCycles /
                                       (double)totalTime * 100.0
                                 : 0.0;
      printf("  %s: %llu cycles (%.2f%%) [%llu hits]", aggregated[i].label,
             (unsigned long long)aggregated[i].totalExclusiveCycles, pct,
...
inline void BeginProfile() { GlobalProfiler::instance().begin(); }

inline void EndAndPrintProfile() { GlobalProfiler::instance().printResults(); }
...
struct ScopedTimer {
  ...
  ScopedTimer(const char *label, u64 bytes = 0)
      : entryIndex(-1), startCycles(0), byteCount(bytes) {
...
#define TIME_BANDWIDTH(label, byteCount)                                       \
  ScopedTimer _scoped_timer_##__LINE__(label, byteCount)
