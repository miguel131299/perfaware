...
#ifdef _WIN32
  void *buffer = VirtualAlloc(nullptr, BUFFER_SIZE, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  if (!buffer) {
    fprintf(stderr, "ERROR: VirtualAlloc failed\n");
    return nullptr;
  }
#else
  void *buffer = mmap(nullptr, BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
...
int main(int argc, char **argv) {
  ...
  if (argc >= 3) {
    direction = argv[2];
    if (strcmp(direction, "forward") != 0 && strcmp(direction, "backward") != 0 && strcmp(direction, "both") != 0) {
      fprintf(stderr, "ERROR: direction must be 'forward', 'backward', or 'both'\n");
      return 1;
    }
  }
